...
try {
    ...
    $sql = "INSERT INTO students (name, fname, address, city, state, contact)
            VALUES (:name, :fname, :address, :city, :state, :contact)";
    ...
    if ($stmt->execute()) {
        echo json_encode(['message' => 'Student record inserted successfully.', 'status' => true]);
    } else {
        echo json_encode(['message' => 'Failed to insert student record.', 'status' => false]);
    }
